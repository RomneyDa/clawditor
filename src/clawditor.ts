...
function resolveOpenclawCandidateRef(candidate: Candidate): string {
...
function ensureCachedOpenclawCheckout(options: CliOptions, candidate: Candidate): string {
...
function ensureCachedCrabboxBinary(options: CliOptions): string {
...
function crabboxBinaryAvailable(openclawRoot: string): boolean {
...
function preflight(options: CliOptions, candidate: Candidate, profile: Profile): JsonObject {
  ...
  if (options.suites.has("crabbox")) {
    ...
    if (!options.dryRun && !crabboxBinaryAvailable(options.openclawRoot)) {
      throw new Error(
        `Crabbox binary not found for ${options.openclawRoot}. Clone openclaw/crabbox next to that checkout or add crabbox to PATH.`,
      );
    }
  }
...
function localBuildDependencyInstallScript(repoVar: string, label: string): string {
...
function runLiveProcess(command: string[], prefix: string, options: CliOptions, opts: JsonObject = {}): Promise<RunResult> {
  return new Promise((resolveRun) => {
    ...
    const providerRunUrls = new Set<string>();
...
function kovaReportHighlights(report: any): string[] {
  ...
  const failingRecords = Array.isArray(report.records)
    ...
    : [];
  const groupedOwners = new Map<string, number>();
  const groupedViolations = new Map<string, number>();
...
function codexAuditFinalVerdict(text: string): JsonObject {
...
function codexAuditPromptText(candidate: Candidate, options: CliOptions, outputDir: string): string {
...
async function runCodexAudit(candidate: Candidate, profile: Profile, options: CliOptions, outputDir: string): Promise<RunResult | null> {
  if (!options.suites.has("codex") || !profile.codexAudit) return null;
  ...
  const startedAt = new Date().toISOString();
  const result: RunResult = await runLiveProcess(command, "codex: crabbox-audit", options, {
    cwd: options.openclawRoot,
    stdin: prompt,
    timeoutMs: codexAuditTimeoutMs,
    heartbeatMs: 30000,
...
function statusText(run: JsonObject): string {
  ...
  if (run.action === "blocked") return `blocked${run.exitCode !== undefined ? ` (exit ${run.exitCode})` : ""}`;
...
function codexSummaryLines(run: JsonObject | null): string[] {
  if (!run) {
  ...
  if (run.modelReason) lines.push(`- model reason: ${run.modelReason}`);
  if (run.startedAt) lines.push(`- started: ${run.startedAt}`);
  if (run.timedOut) lines.push("- timed out: true");
  if (run.prompt) lines.push(`- prompt: ${run.prompt}`);
  if (run.lastMessage) lines.push(`- last message: ${run.lastMessage}`);
...
function verdictFor(summary: JsonObject): string {
  ...
  if (failedDownload || failedKova || failedCrabbox || failedCrabpot || failedCodexProduct) return "LOCAL_FAIL";
